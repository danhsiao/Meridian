"""GENERATED from a frozen spec. Do not hand-edit for a passing test.
    ...
    process:   final_demo_test
    spec_hash: sha256:02aa67855eef5a86887545ee772daf05233340fd6cba05ed84417dec550fdcae
    topo:      ['cha_1', 'art_1', 'art_2', 'art_4', 'art_5', 'art_3', 'pol_1', 'pol_2', 'out_1']

Regenerate with `cli gen --process final_demo_test`.
...
def run(payloads: list[dict[str, Any]], spec_path: str) -> dict[str, Any]:
    ...
    # ── pol_2 (policy: exists_matching, verdict on art_5) ─────
    ...
    for _record in state.records("art_5"):
        ...
        state.verdict("pol_2", "art_5", _record, _ok, f"exists_matching on {_candidate_path}")

    # ── e_6 (art_4 -> art_2, verdicts travel up) ───────
    # Emitted after every policy and before the outputs: propagation reads
    # verdicts, so every check has to have run first.
    state.propagate("art_4", "art_2")
